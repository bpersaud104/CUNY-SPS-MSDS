...
# The population distribution seems fairly normal and slightly right skewed. There is a mean of 1500, the min is 334 and the max is 5642.
...
# The distribution for this sample is fairly normal and slightly right skewed. It is closely similiar to the distrbution of the population since the sample data was randomly selected from the distribution of the population.
...
# The mean of sampl2 is different from the mean of samp1. Samp2 has a mean of 1465.66 and samp1 has a mean of 1451.04.
...
# The sample with size 1000 would provide a more accurate estimate of the population mean. The larger the sample size the closer the mean will be to the population mean.
...
# There are 5000 elements in sample_means50. The sampling distribution is normal with most of the data at the center. The center is the means that are around 1500, the population mean. If we collected 50,000 sample means the distribution would change slightly as it would approach a more perfectly shaped normal distribution.  
...
# There are 100 elements in sample_means_small. Each  element represents the mean of a sample from the 100 samples of size 50 that was taken from the population. 
...
# The center gets closer and closer to the population mean when the sample size is larger. The spread becomes more compressed and makes the distribution look less skewed.  
...
```{r}
samp6 <- sample(price, 50)
mean(samp6)
```

# The best point estimate of the population mean is 181485.1.
...
sample_means50 <- rep(NA, 5000)
for (i in 1:5000) {
  samp7 <- sample(price, 50)
  sample_means50[i] <- mean(samp7)
}
hist(sample_means50)
```

# This sampling distribution looks to be have the shape of a normal distribution. Based on this sampling distribution, I would guess that the mean home price of the population to be around 180000.

```{r}
mean(price)
```

# The population mean is 180796.1.
...
for (i in 1:5000) {
  samp8 <- sample(price, 150)
  sample_means150[i] <- mean(samp8)
}
hist(sample_means150)
```

# This sampling distribution looks to also have the shape of a normal distribution. Compared to the sampling distribution for a sample size of 50, the sampling distribution for a sample size of 150 has a spread that is more compressed. Based on this sampling distribution, I would guess the mean sale price of homes in Ames to be around 181000. 
...
# Of the sampling distributions from 2 and 3, the distribution from 3 has a smaller spread. This is the one with the sampling distribution for a sample size of 150. We would prefer a small spread if we're concerned with making estimates that are more often close to the true value since the small spread comes from the larger sample size, and the larger sample size shows means that are closer to the population mean. 