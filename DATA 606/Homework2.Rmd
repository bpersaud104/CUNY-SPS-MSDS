...
# Histogram (a) is unimodel and looks to almost be symmetrical and could be normally distributed. It matches with box plot (2).
...
# Histogram (c) is unimodel and is right skewed. It matches with box plot (1).
...
> We write *alive* on _______28___ cards representing patients who were 
alive at the end of the study, and *dead* on ___75______ cards 
representing patients who were not. Then, we shuffle these cards and split them 
into two groups: one group of size _____69____ representing treatment, and 
...
times to build a distribution centered at _____45/69 - 30/34____. Lastly, we calculate 
the fraction of simulations where the simulated differences in proportions are 
___-0.230179______. If this fraction is low, we conclude that it is unlikely to 
...
treatment_dead_ratio <- x[transplant == "treatment" & survived == "dead"]$count / sum(x[transplant == "treatment"]$count)
control_dead_ratio <- x[transplant == "control" & survived == "dead"]$count / sum(x[transplant == "control"]$count)
...
# iii. The simulaton results shown below suggest that the transplant program was effective in showing that a patient who gets a heart transplant is more likely to live longer than if the patient does not get a heart transplant.