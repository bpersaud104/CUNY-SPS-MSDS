...
# A streak length of 1 means there was one hit then a miss. A streak of length zero means that there are no hits just one or more miss.
...
# The distribution of Kobe's streak lengths is right skewed. His typical streak length was zero. His longest streak of baskets was a streak length of 4.  
...
```
# 24 out of 100 flips were heads.
...
```{r}
sim_streak_length <- calc_streak(sim_basket)
barplot(table(sim_streak_length))
```
...
# The distribution is right skewed. The typical streak length is a streak length of zero. The player's longest streak length in 133 shots is a streak length of 6. 
...
```{r}
sim_basket_second <- sample(outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55))
table(sim_basket_second)
```

```{r}
sim_streak_length_second <- calc_streak(sim_basket_second)
barplot(table(sim_streak_length_second))
```

# The second simulation is somewhat similiar. This time 69 shots were hit and 64 were miss. More shots were made but the streak length is similiar compared to the first simulation. The distribution is still right skewed with typically a streak length of zero is made, the same as the first simulation. The highest streak length is 4, the first simulation's highest streak length was six. . 
...
# Kobe Bryant's distribution of streak lengths is pretty similair to the distribution of streak lengths for the simulated shooter. Both distributions are right skewed with a streak length of zero being the typical streak length to expect from both. I do not have evidence showing the hot hand model fits Kobe's shooting patterns. This is because his his distribution of streak lengths is closely related to the distribution of streak lengths for the simulated shooter, who is supppose to be a simulation of an independent shooter.