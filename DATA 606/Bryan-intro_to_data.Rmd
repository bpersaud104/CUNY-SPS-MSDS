...
# There are 20,000 entries in this dataset. There are nine variables. genhlth(categorial, ordinal), exerany(categorial, nominal), hlthplan(categorial), smoke100(categorial) height(numerical, continuous) weight(numerical, continuous), wtdesire(numerical, continuous), age(numerical, continuous), and gender(categorial)
...
# The mosaic plot reveals about smoking habits and genders that more males smoke at a higher rate than females.
...
```
# There is a positive correlation between people's weight and the weight they want.
...
# wdiff is numeric. If wdiff is 0 then that means the person is at the weight they want to be at. If wdiff is positive then that means the person wants to gain weight. If wdiff is negative then that means the person wants to lose weight.
...
boxplot(wdiff)
```
# Based on the information below, we can see that more people are looking to lose weight. The median is -10, so at least half of the people in this data set want to lose 10 pounds.

-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
```{r}
wdiff_gender <- data.frame(wdiff = wdiff, gender = cdc$gender)
boxplot(wdiff ~ gender, data = wdiff_gender)
```
# We se that more females want to lose weight and men either are fine with their weight or want to gain weight.
...
```
```{r}
one_over_sd <- mean(cdc$weight) + sd(cdc$weight)
one_over_sd
one_under_sd <- mean(cdc$weight) - sd(cdc$weight)
one_under_sd
```
```{r}
length(which(cdc$weight >= one_under_sd & cdc$weight <= one_over_sd)) / nrow(cdc)
```
