...
```{r}
print('According to the plot, there is a steady decline in the baptismal of girls starting in 1640 to 1650. Then starting in 1659 there was an exponential growth in the baptismal of girls until 1700, where there was another decline.')
```
...
```
# According to the summary functions above, the present data set has a much higher count than the arbuthnot data set. They are not on a similiar scale.
...
-   Make a plot that displays the boy-to-girl ratio for every year in the data 
    ...
    response.
```{r}
plot(present$year, present$boys/present$girls, xlab="year", ylab="Ratio of Boys to Girls Births", type = "l")
plot(arbuthnot$year, arbuthnot$boys/arbuthnot$girls, xlab="year", ylab="Ratio of Boys to Girls Births", type = "l")
```
# According to the plot, there is a decline in the boy-to-girl ratio over the years. Arbuthnot's observation about boys being born in greater proportion than girls still holds up because even though there is a decline over the years, the ratio is still over 1.
...
present$year[which.max(present$Totalbirth)]
```
# The year with the total number of births is 1961.
