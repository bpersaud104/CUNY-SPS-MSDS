...
def exercise01():
    ...
    animals = ['cat', 'dog', 'crouching tiger', 'hidden dragon', 'manta ray'] # Assigns 5 animals to a list named animals
...
def exercise02():
    ...
    # ------ Place code below here \/ \/ \/ ------
    animals = ['cat', 'dog', 'crouching tiger', 'hidden dragon', 'manta ray'] # Assigns 5 animals to a list named animals
    len_animals = len(animals) # Assigns the length of the list animals to a variable called lens_animal
    for i in range(0, len_animals): # Assigns the variable i values in the range of 0 to the length of the list animals
            print(animals[i]) # Goes through each item in the list animals and prints each item located in it
    # ------ Place code above here /\ /\ /\ ------
...
def exercise03():
    ...
    # ------ Place code below here \/ \/ \/ ------
    countdown.sort() # Sorts the items in the list countdown
    countdown.reverse() # Sorts the items in the list countdown in reverse order
    the_fifth_element = countdown[4] # Assigns the fifth element in the list countdown to a variable called the_fifth_element
    # ------ Place code above here /\ /\ /\ ------
...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    # ------ Place code below here \/ \/ \/ ------
    temperatures = temperatures + list(more_temperatures) # Adds all items in more_temperatures to the list temperatures
    temperatures = temperatures + list(iot_sensor_points.values()) # Adds the values in iot_sensor_points to a list and adds them to the list temperatures
    temperatures = temperatures + [a, b, c, d, e] # Adds the values stored in variables a, b, c, d, and e to the list temperatures
    temperatures.sort() # Sorts the items in the list temperatures
    temperatures.reverse() # Sorts the items in list temperatures in reverse order
    for i in (range(4, len(temperatures), 5)):
        samples.append(temperatures[i]) # Assigns every fifth reading in the list temperatures to a list called samples
    copy_of_samples = samples[:] # Assigns all items in the list samples to a list called copy_of_samples
    samples.sort() # Sorts the items in the list samples 
    # ------ Place code above here /\ /\ /\ ------
...
def exercise05(n):
    ...
    if n == 1: # Return n if n = 1
        ...
        return n * exercise05(n - 1) # Loops through function and multiplies current n value to n - 1 and continues to do this until function reaches n = 1
    # ------ Place code above here /\ /\ /\ ------
...
def exercise06(n):
    ...
    # ------ Place code below here \/ \/ \/ ------
    length_n = len(n) # Assigns the length of list n to a variable called length_n
    sum_n = sum(n) # Assigns the sum of each element in n to a variable called sum_n
    average_n = sum_n / length_n # Assigns the average of the elements in list n to a variable called average_n
    # ------ Place code above here /\ /\ /\ ------
...
def exercise07(n):
    ...
    # ------ Place code below here \/ \/ \/ ------
    for i in range(len(n) - 1): # Assigns the variable i the values in the range of the length of the list n - 1
        if n[i] == n[i + 1]: # Checks if the current item in n is equal to the next available item in n and returns True if they are equal
            return True
    else:
        return False # Returns false if for all values of i there are no two items in the list n that are equal to each other
    # ------ Place code above here /\ /\ /\ ------
...
def exercise08(s):
    ...
    int_s = int(s) # Converts the string s to an int type
    float_s = float(s) # Converts the string s to a float type
...
def exercise09():
    ...
    # ------ Place code below here \/ \/ \/ ------
    for i in (range(0, 11)): # Assigns the variable i the values in the range of 0 to 10
        dogs.append(str(dog_media.content)) # Adds the value in dog_media.content as a string to a list called dogs
        dog_media = r.get(url=url) # Assigns a new random URL of a dog pic to dog_media
    # ------ Place code above here /\ /\ /\ ------
...
def exercise10(sentence):
    ...
    # ------ Place code below here \/ \/ \/ ------
    string = sentence.swapcase() # Assigns lower case letters as upper case letters and vice versa to a string called string
    string = string.replace(" ", "_") # Replaces any spaces in the string called string with an underscore _
    reversed = string[::-1] # Takes the values in string and puts them backwards in a string called reverse
    # ------ Place code above here /\ /\ /\ ------
    return reversed
