...
```{r}
hist(mdims$hgt, main = "Men's Heights", xlab = "cm")
```

```{r}
hist(fdims$hgt, main = "Women's Heights", xlab = "cm")
```

# Both distributions appear to be normal. The Men's distributions has a slightly higher mean than the Women's distributions.
...
# Yes, based on this plot it does appear to follow a nearly normal distribution. 
...
# Majority of the points fall on the line, there are some that deviate from the line. This simualted plot is almost similiar to the probability plot for the real data. 
...
# The normal probability plot for 'fdims$hgt' does look similiar to the plots created for the simualted data. Yes, the plots provide evidence that the femlae heights are nearly normal since the simuated data looks to be almost similar to the real data.
...
# Based on the plots, female weights does not seem to come from a normal distribution but appears to come from a right skewed distribution.
...
# Question 1: What is the probability that a randomly chosen female is taller than 175cm?
...
# Question 2: What is the probability that a randomly chosen female weighs less than 60kg?

```{r}
fwgtmean <- mean(fdims$wgt)
fwgtsd <- sd(fdims$wgt)
pnorm(q = 60, mean = fwgtmean, sd = fwgtsd)
```
...
# Female heights had a closer agreement than female weights. Between both methods for heights the probability for theoretical normal distribution was 6.09% and for empirical distribution was 7.69%. For weights the probability for theoretical normal distribution was 47.5% and for empirical distribution was 54.61%.
...
# Plots C and D have a slight stepwise pattern because I'm guessing it has to do with the integer values in the data set being rounded to the nearest unit. Also majority of the integers are probably commonly found in the data.
...
# Based on the normal probability plot, the female knee diameter variable appears to be right skewed.

```{r}
hist(fdims$kne.di, main = "Female Knee Diameter", xlab = "cm")
```

# The histogram confirms that the female knee diameter variable appears to be right skewed.
